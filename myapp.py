...
app.add_url_rule('/countstaticdetail',view_func=CountStaticWeekDetail.as_view('countstaticdetal2'))
...
def logit(req):
	if not request.url.find('notifyget')>0:
		notify_m(content='动作：'+str(request.url)+"  地址："+str(request.remote_addr),status=1)
