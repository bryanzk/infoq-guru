...
class RssWeiboShare(MethodView):
	def get(self):
		...
		if _weibo:
			...
			author=_img.fname
		content=_rss.description.replace(" ","").replace('\br',"")
		img=_img.img
		ctype=helper._get_cats(guid)
		return render_template('editor_weibo_share.html',ctype=ctype,title=title,guid=guid,content=content,author=author,author2=_img.fname,img=img)
